...
onMounted(async () => {
  try {
    ...
    const resp = await fetch(`/api/lista/getPeliculasDeLista/${listaId}`)
    const respLista = await fetch(`/api/lista/getLista/${listaId}`)
...
const addListe = async () =>{
        try {
          ...
          const dataLista = await axios.post(`/api/lista`, {
...
const eliminarDeLista = async (movie) => {
  try {
    const listaId = route.params.id;
    await axios.delete('/api/lista/eliminarPelicula', {
