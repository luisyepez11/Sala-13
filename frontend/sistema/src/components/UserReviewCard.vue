...
function navigateToDetails() {
	if (movie.value && movie.value.id) {
		router.push(`/details/${movie.value.id}`);
	}
}
...
<template>
	<div v-if="movie" class="review-entry-card">
		<div class="movie-poster-container" @click="navigateToDetails">
		...
		<div class="review-content">
			...
			<div class="rating-stars">
				<svg v-for="n in 5" :key="n" class="star-icon" :class="{ 'filled': n <= review.valoracion }" fill="currentColor" viewBox="0 0 20 20">
