...
const fetchMovies = async () => {
	try {
		const response = await axios.get("/api/pelicula")
		...
		console.error("Error al cargar las películas del carrusel:", error);
	}
}
		...
		<div class="carousel-wrapper">
			<div class="slides-wrapper">
				<div 
					...
					@click="goToMovieDetails(slide.id)"
				>
			...
			<button class="nav-button nav-prev" @click.stop="prevSlide" aria-label="Anterior">
				<svg class="nav-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 19l-7-7 7-7"/>
				</svg>
			</button>
			
			<button class="nav-button nav-next" @click.stop="nextSlide" aria-label="Siguiente">
				<svg class="nav-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
